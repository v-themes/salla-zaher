import { setTranslations, expiredStorage, checkLocalStorage, getMobileOperatingSystem } from './helpers';
...
export default {
        ...
        salla.event.on("product::add-to-compare", (response) => {
            ...
            setTimeout(() => {
                ...
                if (compareProductIds.includes(productId)) {
                    salla.notify.info(setTranslations("المنتج موجود بالفعل", 'Product already exists'));
                } else {
                    if (compareProductIds.length >= 4) {
                        salla.notify.error(setTranslations("لا يمكن إضافة أكثر من أربعة منتجات", "You can't add more than four products to compare"));
                    } else {
                        salla.notify.success(setTranslations("تم إضافة المنتج إلى المقارنة بنجاح", "Product added to compare"));
    ...
    removeProductFromCompare() {
        salla.event.on("product::remove-from-compare", (response) => {
            ...
            if (compareProductIds.includes(productId)) {
                ...
                salla.notify.success(setTranslations("تم حذف المنتج من المقارنة بنجاح", "Product removed form compare"));
                updateCompareIndicator();
                if (compareProductIds.length === 0) {
                    document.querySelector("#compare-products-alert").classList.remove("hidden");
                    document.querySelector('#compare-products-container .products-wrapper').classList.add("hidden");
                }
            } else {
                salla.notify.info(setTranslations("المنتج لم يعد موجود بالمقارنة", "Product doesn't exist in compare"));
            }
        });
    },
    ...
    cartItemDeleted() {
        salla.cart.event.onItemDeleted((response) => {
            ...
            salla.storage.set('cart_items', response.data.cart.items.map(item => item.product_id.toString()));
        });
    },
    ...
    registerMenuCategoriesComponent() {
        ...
        if (!menu_categories) {
            salla.product.categories().then((response) => {
                ...
                if (salla.url.is_page('index')) this.registerCategoriesComponent(response.data);

                expiredStorage('menu_categories', response.data);
            });
        } else {
            if (salla.url.is_page('index')) this.registerCategoriesComponent(menu_categories);
    ...
    fillMenuCategories(categories) {
        ...
        categories.filter((category) => category.name).forEach(categoryItem => {
            if (categoryItem.sub_categories && categoryItem.sub_categories.length > 0) {
                ...
                categoryItem.sub_categories.filter((subCategory) => subCategory.name).forEach(subCategory => {
                    htmlContentToRendered += `<li><a href="${subCategory.url}">${subCategory.name}</a></li>`;
                });
                if (categoryItem.sub_categories.length > 1) {
                    htmlContentToRendered += `<li><a href="${categoryItem.url}">${setTranslations('عرض الكل', 'Show all')}</a></li>`;
                }
        ...
        subCategoryHeading.forEach(subCategoryElement => {
            subCategoryElement.addEventListener('click', () => {
                ...
                subCategoryElement.closest('li').querySelector('ul').classList.toggle('show-sub-menu-item');
            });
        });
    },
    registerCategoriesComponent(storedCategories) {
        const categoriesComponents = document.querySelectorAll('.js-categories-component');
    ...
    registerCategoryUrls(categories, tags) {
        tags.forEach(tag => {
            ...
            if (categories) tag.setAttribute('href', categories?.find((category) => category.id_ == categoryId)?.url ?? '');
        });
    },

    // ? ZAHER-COLLECTIONS HANDLER
    registercollectionComponent() {
        const collectionComponents = document.querySelectorAll('.js-collection-component');
        collectionComponents.forEach(async (comp, idx) => {
            ...
            for (let item of collectionsItms) {
                ...
                if (productData) {
                    if (item.classList.contains("style-1")) this.renderProductStyle1(item, productData);
                    if (item.classList.contains("style-2")) this.renderProductStyle2(item, productData);
                } else {
    ...
    async getProductData(item, key) {
        const productId = item.getAttribute("data-id");
        salla.product.getDetails(productId, ['images', 'category']).then((res) => {
            if (res.status != 200) return;
            // Render product data
            if (item.classList.contains("style-1")) this.renderProductStyle1(item, res.data);
            if (item.classList.contains("style-2")) this.renderProductStyle2(item, res.data);
    ...
    renderProductStyle1(item, data) {
        ...
        const { id, name, status, is_available, type, regular_price, sale_price, is_on_sale, images, image
        } = data;
        ...
        cardInfo.innerHTML += `
            ...
            ${is_on_sale ? `
                <span class="text-base text-gray-400 line-through leading-none">${salla.money(regular_price)}</span>
                <span class="text-lg text-primary font-bold leading-none">${salla.money(sale_price)}</span>
            ` :
                `
                <span class="text-lg text-primary font-bold leading-none">${salla.money(regular_price)}</span>
            `}
            </div>
        `;
        miniCard.innerHTML += `
    ...
    indicatorHandler(item) {
        ...
        const topPosition = (topAttr > 0 ? topAttr : Math.floor(Math.random() * 70) + 10) + "%";
        const leftPosition = (leftAttr > 0 ? leftAttr : Math.floor(Math.random() * 70) + 10) + "%";
    ...
    renderProductStyle2(item, data) {
        ...
        // Update thumbs
        const thumb = document.querySelector(`.js-collection-slider img[data-id="${id}"]`);
    ...
    getAppsUrls() {
        const appsTags = document.querySelectorAll("footer .footer-wrapper .store-info .mobile-apps a");
        if (!appsTags) return;

        const appsLinks = {};
        appsTags?.forEach(app => {
            const appType = app.querySelector("img").alt;
            appsLinks[appType] = app.href;
        });
        return appsLinks;
    },
    detectSuitableApp() {
        const innerApp = document.querySelector(".app-inner"),
            apps = document.querySelector('.suggest-mobile-app'),
            closeBtn = apps?.querySelector('.close');

        const mobileType = getMobileOperatingSystem();
        if (mobileType == "unknown") return;

        const linkTag = apps.querySelector("button a");
        const appsUrls = this.getAppsUrls();

        let appUrl = '';
        if (mobileType == "Android") {
            if (!appsUrls['googleplay']) return;
            appUrl = appsUrls['googleplay'];
        }

        if (mobileType == "iOS") {
            if (!appsUrls['appstore']) return;
            appUrl = appsUrls['appstore'];
        }

        linkTag.setAttribute('href', appUrl);

        innerApp.classList.add('show-app-suggest');
        closeBtn.addEventListener('click', () => innerApp.classList.remove("show-app-suggest"));
        window.addEventListener('scroll', () => innerApp.classList.toggle('show-app-suggest', window.scrollY <= 140));
    },
};