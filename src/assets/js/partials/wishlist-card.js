    ...
    this.innerHTML = `
        <div class="flex items-center mb-4 sm:mb-0">
          <a href="${this.product.url}" class="product-entry__image">
            <img class="object-cover w-full h-full lazy" data-src="${
            ...
            <div class="w-full center-between">
              ${
                ...
                <div class="space-x-1 rtl:space-x-reverse">
                  <h4 class="inline-block text-sm font-bold text-red-400">${salla.money(
                    this.product.sale_price
                  )}</h4>
                  <span class="text-sm text-gray-500 line-through">${salla.money(
                    this.product.regular_price
                  )}</span>
                </div>
              `
                  : `
                <h4 class="text-sm font-bold">${salla.money(
                  this.product.price
                )}</h4>
              `
              }
        ...
        <div class="flex items-center space-x-4 rtl:space-x-reverse">
          <salla-add-product-button product-status="${
            this.product.status
          }" product-id="${this.product.id}" product-type="${
      this.product.type
